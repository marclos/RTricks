...
library(xtable)
library(here)
source(here("04_Regional_Climate_Trends", "Guides", "Guide3functions.R"))
@
...
We would want to evaluate the trends for TMIN and PRCP too! You can use the Environment tab in Rstudio to see the trend results for all three variables withut needing any code. 
