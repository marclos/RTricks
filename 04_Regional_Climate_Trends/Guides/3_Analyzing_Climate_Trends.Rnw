...
Run the \href{https://github.com/marclos/RTricks/blob/master/04_Regional_Climate_Trends/Guides/Guide3functions.R}{Guide3functions.R} code and the functions will be loaded into your environment automatically and are designed to help you analyze your statio data. 
...
\section{Analyzing Monthly Trends}
...
\item[Example of how to use the function] Here's an example using the list of station dataframes anomalies. Note: I have spelled this wrong!
...
<<>>=
par(mar=c(5,5,3,4), las=1)
      
plot(TMIN.a ~ Ymd, data=subset(USC00042294.anamolies$TMIN, 
    MONTH==1), las=1, pch=20, cex=.5, col="grey", 
    ylab="Temp Anamoly (°C) ", 
    main="", 
    xlab="Year", frame.plot=TRUE)
mtext("Minimum Daily Temperature Trend for January", cex=1.3, side=3, line=1)

subtitle <- "Trend Line: 2.0/100 yrs; p-value = 0.004, r2 = 0.006"
mtext(subtitle, side=3, line=0, cex=1.0, col="red")

USC00042294.lm = lm(TMIN.a ~ Ymd, 
          data=subset(USC00042294.anamolies$TMIN, MONTH==1))
...
station[sample(1:nrow(station), 10), c(9, 1,3,4,10, 11, 12)]
...
station$Season <- ifelse(station$MONTH %in% c(3,4,5), "Spring", 
                  ...
                  "Winter")))
PRCP.Decade.mean <- aggregate(VALUE ~ Season + Decade, 
          data =  subset(station, subset=ELEMENT=="PRCP"), mean, na.rm=T)
PRCP.Decade.sd <- aggregate(VALUE ~ Season + Decade, 
          data =  subset(station, subset=ELEMENT=="PRCP"), sd, na.rm=T)
...
library(tidyr)
PRCP.Decade.mean <- spread(PRCP.Decade.mean, Season, VALUE)
@
