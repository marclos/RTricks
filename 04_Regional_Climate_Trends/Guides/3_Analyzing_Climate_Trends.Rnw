...
\section{Function to Analyze Monthly Trends}
...
\subsection{Functions to Evaluate Trends}
...
# separate 
names(USC00042294.anomalies)

str(USC00042294.anomalies)


USC00042294TMAX = USC00042294.anomalies$TMAX
names(USC00042294TMAX)
str(USC00042294TMAX)
...
test.lm <- lm(TMAXa ~ YEAR, Data = USC00042294TMAX$MONTH==1)
...
lm(USC00042294TMAX$TMAXa ~ USC00042294TMAX$YEAR)


USC00042294TMAX$TMAX.a <- USC00042294TMAX$TMAX.a
# determine the lm for each month
  

test.lm <- lm(TMAX.a ~ YEAR, Data = USC00042294TMAX$MONTH==1)

hist(USC00042294TMAX$TMAXa, main="TMAX.a", xlab="TMAX.a")
hist(USC00042294TMAX$YEAR, main="TMAX.a", xlab="TMAX.a")


aggregate(TMAX.a ~ YEAR, Data = USC00042294TMAX, by=list(Month=MONTH), FUN=mean)




# Monthly Eval Stats function use lm()
MonthEvalStats <- function(month, data) {
  lm(TMAX.a~YEAR, data)
}


lapply(USC00042294.anomalies, function(x) MonthEvalStats(1, data=x))
...
<<EvaluationMonthlyTrends, eval=FALSE>>=
...
<<EvaluatingMonthDelta, eval=FALSE>>=
