...
Run the \href{https://github.com/marclos/RTricks/blob/master/04_Regional_Climate_Trends/Guides/Guide3.R}{Guide3.R} code and the functions will be loaded into your environment automatically and are designed to help you analyze your statio data. 
...
\subsubsection{Rainfall Probability Distributions over Time}
...
@
I don't see a pattern!  But maybe we need to use a statistical approach!
...
\begin{enumerate}
\item The first plot is the TMIN Trend Line (month of X?)

<<echo=TRUE, eval=TRUE>>=
TMIN in the March, at station USC00040693
station = subset(USC00040693.anomamly, subset(MONTH==3, ELEMENT=="TMIN")
