...
Using a list of active weather stations in the United States, you  will download and process the data to create a time series of temperature anomalies. These datasets will be used to develop trend analysis to frame the policy implications for the region.
...
cleandataframe.fun <- function(x, y="TMAX"){
  x$VALUE = x$VALUE/10
  ...
  x.monthly = aggregate(VALUE ~ MONTH + YEAR, 
                   data = subset(x, ELEMENT == y), mean)
  x.normals = subset(x, Ymd >= "1961-01-01" & Ymd <= "1990-12-31")
  x.normals.monthly = aggregate(VALUE ~ MONTH, 
                   data = subset(x.normals, ELEMENT == "TMAX"), mean)
  names(x.normals.monthly) <- c("MONTH", "NORMALS")
  x.anomaly = merge(x.monthly, x.normals.monthly, by = "MONTH")
  x.anomaly$ANOMALY = x.anomaly$VALUE - x.anomaly$NORMALS
  return(x.anomaly)
}
@
...
\section{Monthly Anomalies Linear Regression}

<<Monthly_summarystats, eval=TRUE, echo=FALSE>>=
