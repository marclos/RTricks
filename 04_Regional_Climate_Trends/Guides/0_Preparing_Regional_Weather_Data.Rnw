...
\date{\today~(ver. 0.5)} % without \date command, current date is supplied
...
\begin{enumerate}
  \item Data Collection (Download data from the web and read into R.)
  \item Data Processing (Clean and organize data for analysis.)
  \item Data Analysis (Use R to analyze data (means, trends, etc))
  \item Data Visualization (Create visualizations of data)
  \item Communicating Project Results (Create a video that explains the data and the results of the analysis.)
\end{enumerate}
