...
\hlkwd{write.csv}\hlstd{(stations.active.oldest,} \hlstr{"stations.active.oldest.csv"}\hlstd{)}


\hlstd{StationStates} \hlkwb{=} \hlkwd{unique}\hlstd{(Station.sel}\hlopt{$}\hlstd{STATE)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in unique(Station.sel\$STATE): object 'Station.sel' not found}}\begin{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{state.df} \hlkwb{=} \hlkwd{subset}\hlstd{(Station.sel,} \hlkwc{subset}\hlstd{=STATE}\hlopt{==}\hlstd{my.state)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in subset(Station.sel, subset = STATE == my.state): object 'Station.sel' not found}}\begin{alltt}
\hlcom{#subset(Stations, subset=STATE %in% state.df$STATE)}
\end{alltt}
\end{kframe}
\end{knitrout}
