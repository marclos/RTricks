...
Run the \href{https://github.com/marclos/RTricks/blob/master/04_Regional_Climate_Trends/Guides/Guide2.R}{Guide2.R} code and the functions will be loaded into your environment automatically.
...
The If the data are not in the R environment, you can read them in using the following but you have the uncompressed files, you can read them in using the following function:
...
%You should see object named after the stations you read in. For example, if you read in the station data (e.g. \Sexpr{my.inventory$ID[1]}, you should see an object named station1 in your environment.
...
<<echo=TRUE, results='hide'>>=
USC00042294b <- ConvertUnits.fun(USC00042294a)
@
...
<<echo=TRUE, results='hide'>>=
USC00042294.anomolies <- MonthlyAnomalies.fun(USC00042294.monthly, USC00042294.normals)
@
...
\begin{itemize}
  \item \verb|USC00042294.anomolies[[1]]| for TMAX
  \item \verb|USC00042294.anomolies[[2]]| for TMIN
  \item \verb|USC00042294.anomolies[[3]]| for PRCP
\end{itemize}
...
plot(TMAX.a ~ Ymd, data=USC00042294.anomolies[[1]], type="p", col="red", pch=19, cex=.2, las=1, xlab="Year", ylab="Temperature Anomaly (C)", main="Monthly Maximum Temperature Change")
plot(TMIN.a ~ Ymd, data=USC00042294.anomolies[[2]], type="p", col="blue", pch=19, cex=.2, las=1, xlab="Year", ylab="Temperature Anomaly (C)", main="Monthly Minimum Temperature Change")
plot(PRCP.a ~ Ymd, data=USC00042294.anomolies[[3]], type="p", col="green", pch=19, cex=.2, las=1, xlab="Year", ylab="Precipitation Anomaly (C)", main="Monthly Precipiation Change")
@
\end{figure}
...
Also, I like to move the final products into csv files. Here's the function to do that: coming soon

<<cleanup, echo=FALSE, eval=FALSE>>=
CleanUp.fun(datafolder, station1)
@
...
\subsection{QA/QC: Troubleshooting}
