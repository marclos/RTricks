...
\subsubsection{What is GHCN-Daily?}
...
Also, I like to move the final products into csv files. Here's the function to do that: 

<<cleanup, echo=TRUE, eval=FALSE>>=
CleanUp.fun(datafolder, USC00042294.anomalies, "USC00042294")
@

It's clunky, but not that imporant at this point, so I'll work on it later.
...
If you have too many stations that don't have enough data, we'll need to download additional stations. I can show you a way to decipher that ahead of time if you'd like to know. Otherwise, I suggest you double the number of stations selected in the code that generated my.inventory. 
