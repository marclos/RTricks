...
library(xtable)
filename = "/home/mwl04747/RTricks/00_Admin/Random_IDs/EA30S2026.csv"
import = read.csv(filename); names(import)
enrollment = nrow(import); enrollment
import = subset(import, select=c("Student", "Email"))
@
...
print(xtable(enrolled[,c(1, 3:7)], caption='Random Numbers for EA30, Spring 2025', label='tbl:numbers'), scalebox=1.0, caption.placement = "top")
@
...
<<echo=FALSE>>=
# Sorted For Marc!
R1 = enrolled[,c(1,3)]; R2 = enrolled[,c(1,4)]; R3 = enrolled[,c(1,5)]; 
R4 = enrolled[,c(1,6)]; R5 = enrolled[,c(1,7)]
names(R1) = names(R2) = names(R3) = names(R4) = names(R5) = c("Student", "RandomNo")
enrolled_long = rbind(R1, R2, R3, R4, R5)
enrolled_long = enrolled_long[order(enrolled_long$RandomNo),]
write.csv(enrolled_long, file="/home/mwl04747/RTricks/00_Admin/Random_IDs/EA30F25_Roster_Random.csv", row.names=FALSE)
@
