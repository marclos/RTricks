...
temp.csv <- "/home/mwl04747/RTricks/02_pHake_Lake/Fall 2025 Temperature Profile Data Entry - Aug 2025.csv" # change this to your path...
...
# sort by depth
temp = temp[order(temp$Depth),] # sort the data frame by depth
...
```{r Addtitle}
plot(Depth~Temperature, data=temp, las=1, ty="b", ylim=c(6,0), 
     main="pHake Lake Temperature Profile (Aug 26, 2025)")
```

5. Define units with axes labels:
```{r Addlabels}
plot(Depth~Temperature, data=temp, las=1, ty="b", ylim=c(6,0), ylab="Depth (M)", 
xlab="Temperature (°C)", main="pHake Lake Temperature Profile (Aug 26, 2025)")
```

I had to copy and paste the degree symbol, there are better ways to do that, but 
it's complicated -- so do this to keep it simple for now. 
...
## Add Oxygen as a Second Graph

1. Create a new R chunk below the temperature profile plot.
2. Create a plot of Oxygen vs Depth using the same format as the temperature profile.
```{r OxygenProfile, echo=TRUE, eval=FALSE}
mfrow=c(2,1) # set up for two plots in one column
plot(Depth~Temperature, data=temp, las=1, ty="b", ylim=c(6,0), ylab="Depth (M)",
xlab="Temperature (°C)", main="pHake Lake Temperature Profile (Aug 26, 2025)")

plot(Depth~Oxygen, data=temp, las=1, ty="b", ylim=c(6,0), ylab="Depth (M)",
xlab="Oxygen (mg/L)", main="pHake Lake Oxygen Profile (Aug 26, 2025)")
