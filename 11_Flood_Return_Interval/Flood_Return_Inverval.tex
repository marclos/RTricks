...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{splitdata.fun}\hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{annualpeak}\hlstd{,} \hlkwc{splitdate}\hlstd{)\{}

    \hlstd{period1} \hlkwb{=} \hlstd{annualpeak[annualpeak}\hlopt{$}\hlstd{peak_dt} \hlopt{<} \hlstd{splitdate,]}
    \hlstd{period2} \hlkwb{=} \hlstd{annualpeak[annualpeak}\hlopt{$}\hlstd{peak_dt} \hlopt{>=} \hlstd{splitdate,]}
    \hlstd{periodlist} \hlkwb{=} \hlkwd{list}\hlstd{(period1, period2)}
  \hlkwd{return}\hlstd{(periodlist)}
\hlstd{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
