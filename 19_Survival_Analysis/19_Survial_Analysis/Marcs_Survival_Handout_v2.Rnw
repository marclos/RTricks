...
\subsection{Why Survival Analysis is Smart $\checkmark$}
...
Kaplan-Meier curves estimate the probability of **not yet reaching the threshold** over time. **Steeper, earlier drops mean faster growth.**
...
* $\text{HR} > 1$: Treatment reaches the threshold **faster** than A.
* $\text{HR} < 1$: Treatment reaches the threshold **slower** than A.
