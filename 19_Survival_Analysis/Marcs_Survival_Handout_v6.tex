...
\subsection{Option B -- marc's simulated example data}

I think simulated data can provide a great foundation when exploring ``new statistical'' methods. In this case, the data have realistic growth curves with noise, i.e. variation needed for a statistical analysis!

More over, I created 10 replicates -- where some replicates that never hit a threshold.



Here are the first 4 observations from each treatment: 
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# inspect a handful}
\hlstd{df_raw} \hlopt{%>%} \hlkwd{group_by}\hlstd{(treatment)} \hlopt{%>%} \hlkwd{slice_head}\hlstd{(}\hlkwc{n} \hlstd{=} \hlnum{4}\hlstd{)}
\end{alltt}
\begin{verbatim}
## # A tibble: 12 x 5
## # Groups:   treatment [3]
...
##  5 B_1        0 0.00495    -0.0645 B        
##  6 B_1        8 0.213       0.199  B        
##  7 B_1       16 0.0448      0.0604 B        
##  8 B_1       24 0.365       0.342  B        
##  9 C_1        0 0.0587      0.0488 C        
## 10 C_1        8 0.104       0.0963 C        
## 11 C_1       16 0.233       0.217  C        
## 12 C_1       24 0.432       0.420  C
\end{verbatim}
\end{kframe}
\end{knitrout}
...
The next step ti to create some ``end points''. In this case, the end point is simply the end of the experiment. Everyone is censored at that time.
