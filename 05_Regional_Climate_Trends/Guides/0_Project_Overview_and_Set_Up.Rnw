...
\date{\today~(ver. 1.00)} % without \date command, current date is supplied
...
I have created several Guides 1 through 4. Each guide is a Rmarkdown document that explains the steps to complete the project. These guides are designed to be completed in order. %, but it is possible to skip a guide if the data is already available.
...
\begin{figure}
  \caption{A plot of active global weather stations (GHCN-d). Note the increase in stations over time and spatial distribution. It's a story of the industrialization of Europe and the US and colonialiam.}
...
points(LATITUDE~LONGITUDE, data=subset(active.TMAX, subset=FIRSTYEAR<1900), pch=20, cex=.17, col="cornflowerblue")
points(LATITUDE~LONGITUDE, data=subset(active.TMAX, subset=FIRSTYEAR<1890), pch=20, cex=.23, col="blue")
points(LATITUDE~LONGITUDE, data=subset(active.TMAX, subset=FIRSTYEAR<1875), pch=20, cex=.37, col="green")
points(LATITUDE~LONGITUDE, data=subset(active.TMAX, subset=FIRSTYEAR<1865), pch=20, cex=.42, col="red")
legend("bottomleft", legend=c("<1865", "<1875", "<1890", "<1900", ">=1900"), pch=20, col=c("red", "green", "blue", "cornflowerblue", "grey70"), title="In Operation", cex=1, bty="n")
@
\end{figure}

\subsection{Subsetting the GHCN-d Inventory}
...
To ensure we can limit our data to stations that are active, we need to subset the data to include only stations that have data from 2024 and later, however, if we don't find enough stations on the state you selected, I have relaxed the criteria to 2023.
