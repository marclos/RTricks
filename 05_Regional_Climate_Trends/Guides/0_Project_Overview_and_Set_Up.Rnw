...
\date{\today~(ver. 1.01)} % without \date command, current date is supplied
...
\begin{description}
  ...
  \item[Guide 1] Data Collection (Download station inventory from the web and read into R)
...
At this point, I created a DRAFT visual flow chart that displays some of the stages. I will be adjusting this chart as we progress, adding detail. Moreover, I'll refine the R code and project guides if we found ambiguities. 
...
I created several Guides 1 through 5. Each guide is a Rmarkdown document that explains the steps to complete the project. These guides are designed to be completed in order. %, but it is possible to skip a guide if the data is already available.

This particular document has \textbf{no code} for you to run, but documents the source of the data; explains process to obtain the data; and creates a list of stations to be used in Guide 1. Please read this guide as an informational document about our data source. The document includes some insights here about how to find the data and the code that I used to prepare for the project for the class. 
