...
From the Canvas page, go to the Guide3functions.R file and download the file to your computer. Then upload the file to Rstudio directory you are using for the project. ake sure you have an updated version of the file, this one seems the least stable!

Open the file in Rstudio and run the code, using the ``source''. button near the top of the editor window.

Run the \href{https://github.com/marclos/RTricks/blob/master/04_Regional_Climate_Trends/Guides/Guide3functions.R}{Guide3functions.R} code and the functions will be loaded into your environment automatically and are designed to help you analyze your station data.

the Guide3functions.R file is living file -- so I suggest you check on the updates to the file every few days to see if there have been improvments. 
...
\item[Example of how to use the function] Here's an example using the list of station dataframes anomalies. Note: I have often spelled the word anamoly wrong!
...
\begin{verbatim}
USC00042294.trends[USC00042294.trends$ELEMENT=="TMAX",]
\end{verbatim}

\end{description}

<<echo=FALSE, results='asis', fig.cap="TMAX Trends X">>=
xtable(USC00042294.trends[USC00042294.trends$ELEMENT=="TMAX",], caption = "TMAX Trends", label="tab:TMAXtrends")
# print command?
#c(1,7,4:6)], digits=c(0,0,4,2,4,3), caption="TMIN Trends"
# print(TMAX.xtbl)
...
<<>>=
plotTrend2.fun(USC00042294.anomalies, "TMAX", 6)
plotTrend2.fun(USC00042294.anomalies, "TMIN", 7)
plotTrend2.fun(USC00042294.anomalies, "PRCP", 1)
@

\subsection{Acceleraation of Trend in 1975}
...
<<post1974>>=
USC00042294.lm = lm(TMAX.a ~ Ymd, data=subset(USC00042294.anomalies[[1]], subset=MONTH==6))

USC00042294.post1975 = subset(USC00042294.anomalies[[1]], subset= YEAR > 1974)
USC00042294.post1975.lm = lm(TMAX.a ~ Ymd, data=subset(USC00042294.post1975, subset=MONTH==6))

plot(TMAX.a ~ Ymd, data=subset(USC00042294.anomalies[[1]], subset=MONTH==6), pch=20, cex=.5, col="grey", ylab="Temp Anamoly (°C) ",  main="", xlab="Year", frame.plot=TRUE)
     
abline(coef(USC00042294.lm), col="black", lwd=1.5)

points(TMAX.a ~ Ymd, data=subset(USC00042294.post1975, subset=MONTH==6), pch=20, cex=.5, col="red")

new = seq.Date(from=as.Date("1975-01-01"), to=as.Date("2024-12-31"), by="month")
new

predict.lm(USC00042294.post1975.lm, interval="confidence")


lines(USC00042294.post1975$Ymd, USC00042294.post1975.lm$fitted.values, col="red", lwd=1.5)

abline(coef(USC00042294.post1975.lm), col="red", lwd=1.5)
...
<<show_droughtCount.fun, echo=FALSE>>=
...
<<normalPDF, echo=FALSE, eval=FALSE>>=
library(wesanderson)
