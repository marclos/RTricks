...
head(maunaloa)
str(maunaloa)
write.csv(maunaloa, file = "C:\\Users\\mwl04747\\Box\\1. Teaching\\Statistics\\RTricks\\01_Overview_of_R\\maunaloa.csv", eol = "\n", row.names = F)
@
...
<< echo = FALSE, results = 'hide' >>=
filename="C:\\Users\\mwl04747\\Box\\1. Teaching\\Statistics\\RTricks\\01_Overview_of_R\\maunaloa.csv"
...
Okay as of \today, the average is \Sexpr{round(mean(maunaloa$average, na.rm=T),3)}\sidenote{How many significant figures should you report? Have I reported this correctly?  Look up the rule of rounding on the internet to confirm your response.} It will change next month when new is added to the data set to the NOAA dataset.  Now let's calculate the median and standard deviation.\sidenote{How could you remind yourself what the \texttt{na.rm} argument is doing?}
...
<<echo = FALSE, fig = TRUE >>=
par(las=1)
plot(maunaloa$year, maunaloa$average, ylab="ppm", xlab="Year", pch=20, 
main="Yearly Carbon Dioxide Concentrations , \n Mauna Loa, HI" )
@
\end{figure}
