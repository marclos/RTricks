...
**Data source:** NOAA Global Historical Climatology Network â€“ Daily (GHCN-D)  
**Analysis output:** Trend estimates in Â°C/century (temperature) and mm/century (precipitation)  
...
â”œâ”€â”€ NAMESPACE
â”œâ”€â”€ R/
â”‚   â”œâ”€â”€ ClimateNarratives-package.R   # Package docs, dependency checker, startup
â”‚   â”œâ”€â”€ initialize.R                  # initialize_project(), set_config()
â”‚   â”œâ”€â”€ download.R                    # select_stations(), download_stations()
â”‚   â”œâ”€â”€ processing.R                  # load_and_save_stations()
â”‚   â”œâ”€â”€ data_cleaning.R              # fixDates.fun(), fixValues.fun(), coverage.fun()
â”‚   â””â”€â”€ analysis.R                   # MonthlyValues/Normals/Anomalies/Trend functions
â”œâ”€â”€ man/                             # Auto-generated help files
â”œâ”€â”€ vignettes/
â”‚   â””â”€â”€ getting-started.Rmd
â”œâ”€â”€ install_package.R                # One-step installer script
â””â”€â”€ example_workflow.R               # Complete working example
```
...
- **state** â€” Two-letter state abbreviation (e.g., `"CA"`, `"TX"`, `"NY"`)
- **path** â€” Where to create the project. If omitted (recommended), auto-creates `~/ClimateNarratives_XX/` where XX is the state code.
...
**Why the path auto-generates:** Students were not editing path variables, causing files to save to the wrong location. Now `initialize_project("CA")` is all you need â€” no path editing required.
...
- **n** â€” Number of stations to select (50 recommended for good kriging)
- **min_years** â€” Minimum record length in years
- **min_last_year** â€” Station must have data at least through this year
...
- Expect 10â€“30 minutes for 50 stations
- If interrupted, re-run â€” it picks up where it left off
...
Converts NOAA tenths-of-units to standard units: temperatures to Â°C, precipitation to mm.
...
1. **1961â€“1990** (WMO standard)
2. **1971â€“2000** (if #1 lacks coverage)
3. **1981â€“2010** (if #2 lacks coverage)
4. **Full record** (last resort)

At each level, requires all 12 months to have at least `min_years_per_month` years of data. Prints a `message()` when using a non-standard period. Returns `NULL` with a `warning()` if no period has adequate data â€” the station is then skipped by downstream functions.
...
Runs the full pipeline (clean â†’ normals â†’ anomalies â†’ trends) for every station. Skips stations that return NULL at any step and reports a summary at the end.
...
- **trend_var** â€” Column name, e.g., `"annual_trend_TMAX"`, `"winter_trend_PRCP"`
- **colors** â€” `"temp"` for temperature palette, `"precip"` for precipitation palette
- **resolution** â€” Grid cell size in degrees (smaller = finer, but slower)
...
| `datafolder`      | `initialize_project()`    | `"Data/"` (relative path â€” works because wd is set)      |
...
â”œâ”€â”€ Data/
â”‚   â”œâ”€â”€ stations.active.oldest.csv       # Full NOAA inventory (cached)
â”‚   â”œâ”€â”€ selected_inventory_CA.csv        # Your 50 selected stations
â”‚   â”œâ”€â”€ downloaded_inventory.csv         # Successfully downloaded stations
â”‚   â”œâ”€â”€ all_stations_raw.RData           # All station data (fast to reload)
â”‚   â””â”€â”€ spatial_trends_CA.RData          # Processed trend data
â”œâ”€â”€ Output/
...
**v0.3.0** (current)
- Added `check_dependencies()` and startup dependency warnings â€” catches missing packages before they cause cryptic errors
- `initialize_project()` auto-generates path from state code (`~/ClimateNarratives_XX/`), sets working directory, uses relative paths â€” students no longer need to edit paths
- `MonthlyNormals.fun()` cascades through fallback periods (1961â€“1990 â†’ 1971â€“2000 â†’ 1981â€“2010 â†’ full record) for states with poor baseline coverage
- `MonthlyAnomalies.fun()` and `monthlyTrend.fun()` handle NULL inputs gracefully â€” skip bad stations instead of crashing
