...
# Example dataset: emissions data for countries
emissions_data <- data.frame(
  Country = c("Country A", "Country B", "Country C"),
  CO2_emissions = c(1000, 1500, 2000),   # in million metric tons
  CH4_emissions = c(50, 40, 60),         # in million metric tons
  N2O_emissions = c(30, 25, 35)          # in million metric tons
)
...
# Calculate GWP-weighted emissions for each gas
...
write.csv(emissions_data, "emissions_gwp_calculations.csv", row.names = FALSE)
